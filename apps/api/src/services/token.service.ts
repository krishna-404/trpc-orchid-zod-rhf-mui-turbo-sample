import envVars from "../configs/config";
...
export const getAccessToken = (user: T_TokenObj, maxExpiryDate?: string | Date) => {
  const accessTokenExpires = addDays(new Date(), envVars.JWT_ACCESS_EXPIRATION_DAYS);
  ...
  return {
    token: jwt.sign(payload, envVars.JWT_SECRET),
